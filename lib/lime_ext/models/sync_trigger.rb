class LimeExt::SyncTrigger < ActiveRecord::Base
  ...
  ##
  # name prefix for 
  def self.rule_prefix
    @rule_prefix ||= "lsrule"
  end

  def lime_survey_dest
    @lime_survey_dest ||= get_survey(sid_dest)
  end
  
  def lime_survey_src
    @lime_survey_src ||= get_survey(sid_src)
  end
  
  def get_survey sid
    sid.present? ? LimeSurvey.find(sid) : nil
  end

  def parser
    @parser ||= LimeExt::SyncTriggerParser.new self.tgname
  end
  ...
  # 
  def rulename
    @rulename ||= new_record? ? '' : self[:rulename]
  end
  ...
  def cols_src_enum except_id=nil
    cols_enum_filter(sid_src, cols.keys, except_id) do |sid|
      LimeQuestion.where(sid: sid).map{|lq| ["#{lq.question} (qid:#{lq.qid})", lq.my_column_name]}
    end
  end
