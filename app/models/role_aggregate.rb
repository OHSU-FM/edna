class RoleAggregate < ActiveRecord::Base
  ...
  def dynamic_int_or_str_sort enum_options
    if enum_options.any?{|o1, o2| o1.to_i.to_s == o1}
      enum_options.sort_by{|e| e.first.to_i }
    else
      enum_options.sort_by{|e| e.to_s }
    end
  end
  ...
  def valid_enum_options qval, qtitle
    ...
    opts = result.select{|x,y|!(x.to_s.empty? || y.to_s.empty?)}.uniq
    return dynamic_int_or_str_sort opts
  end
